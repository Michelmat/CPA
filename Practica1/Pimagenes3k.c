...
#define IMAGEN_SALIDA "lenna-filp3.ppm" //cambiado nombre
           ...
           int n, int m)
{
  ...
  //struct { int r, g, b; } resultado; Comentado para no usar el struct
  int green, red, blue; // cambiaado por el struct de arriba
  int **ppdBloque, v;
  ...
  for (p = 0; p < pasos; p++) {
    for (i = 0; i < n; i++) {
      for (j = 0; j < m; j++) {
        ...
        blue = 0;
        tot = 0;
#pragma omp parallel for reduction(+:red,green,blue,tot) private(l,v)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
          for (l = max(0, j - radio); l <= min(m - 1, j + radio); l++) {
            v = ppdBloque[k - i + radio][l - j + radio];
            red += ppsImagenOrg[k][l].r * v;
            green += ppsImagenOrg[k][l].g * v;
            blue += ppsImagenOrg[k][l].b * v;
            tot += v;
          }
        }
        red /= tot;
        green /= tot;
        blue /= tot;
        ppsImagenDst[i][j].r = red;
        ppsImagenDst[i][j].g = green;
        ppsImagenDst[i][j].b = blue;
      }
    }
    if (p+1 < pasos)
...
#pragma omp parallel //añadido
	nh=omp_get_num_threads(); //añadido
	t1=omp_get_wtime();		  //añadido	
  Filtro(NUM_PASOS, DIST_RADIO, ImgOrg, ImgDst, n, m);
	t2=omp_get_wtime();		//añadido
	printf("Par %d hilos el tiempo de ejecucion es %f\n",nh,t2-t1);  //añadido
