...
int main(int argc, char *argv[])
{
   ...
   * y enviar la x inicial a todos */
    ...
    MPI_Bcast (x,n, MPI_DOUBLE, 0, MPICOMM_WORLD);

/** Lo hemos comentado todo tras aladir el MPO_Scatter y MPI_Bcast*/
 /** if (yo == 0) {
...
MPI_REDUCE(norma, normat, 1, MPI_DOUBLE, MPI_SUM,0,MPI_COMMWORLD);
/*
  if (yo == 0) {
    for (proc = 1; proc < num_procs; proc++) {
