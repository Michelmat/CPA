...
MPI_Irecv(B,w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &req);      /** <------------------ AÑADIDO */    
     /** MPI_Recv(B, w, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD,
               &status);*//
while (MPI_test(&req,&flag,&status), !flag && filas_hechas<h){							/** <----------- AÑADIDO */  
   num_fila = siguiente_fila;   
   z0.b = y1 + iy*num_fila;
      for ( j = 0 ; j < w ; j++ ) {
        z0.a = x1 + ix*j;
