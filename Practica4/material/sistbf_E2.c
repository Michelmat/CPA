...
MPI_Datatype filas
MPI_Type_vector(nBloque,nN,nN*np,MPI_DOUBLE,&filas)
MPI_Type_commit(&filas)

if (nId == 0) {
	MPI_Sendrecv(&ppdMat[0][0],1,filas,0,200,&ppdMatLcl[0][0],((nN/np)*nN),MPI_DOUBLE,0,200,MPI_COMM_WORLD,&status);
	for (i =1, i<np,i++)
		MPI_Send(&ppdMat[i][0],1,filas,i,201,MPI_COMM_WORLD);
}else{
	MPI_Recv(&ppdMatLcl[0][0],((nN/np)*nN),MPI_DOUBLE,0,201,MPI_COMM_WORLD,&status);
}
MPI_Type_free(&filas);
...
//for i=0 hasta nN-1 
//for (i = 0,i <= nN-1, i++)
//Si nId==0 /*P0 ejecuta este código*/
//if (nId == 0) {
//Si la fila ppdMat[i] está en P0 
//	if (i % nP ==0){ 
//Copiar ppdMat[i] en una fila local de ppdMatLcl 
//	  MPI_Sendrecv(ppdMat[i],nN,MPI_DOUBLE,0,200,ppdMatLcl[i/np],nN,MPI_DOUBLE,0,200,MPI_COMM_WORLD,&status);
//}
//Si no 
//	else{
//Envíar ppdMat[i] al proceso que la debe tener
//	 MPI_Send(ppdMat[i],nN,MPI_DOUBLE,i%np,201,MPI_COMM_WORLD);
//}
//}
//Si no /*código ejecutado por el resto*/
//else{
//Recibir de P0 la fila ppdMat[i] almacenándola en la matriz local ppdMatLcl
//MPI_Recv(ppdMatLcl[i/np],nN,MPI_DOUBLE,0,201,MPI_COMM_WORLD);
//}
